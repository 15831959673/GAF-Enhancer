...
class Diffusion(L.LightningModule):
  ...
  def on_train_epoch_end(self):
    if self.first_epoch:
      print("Skipping early stopping on the first epoch.")
      self.first_epoch = False  # 标记已经完成第一次训练
      return  # 直接返回，跳过早停计算
    # 每10轮结束时触发一次
    if (self.trainer.current_epoch + 1) % 10 == 0:  # 如果已经过了10轮
      print(f"Epoch {self.trainer.current_epoch}: Early stopping due to unconditional stop condition.")
      self.trainer.should_stop = True  # 设置模型训练停止标志
  ...
  def _sample(self, num_steps=None, eps=1e-5, eval_sp_size=None):
    ...
    x_argmax = torch.nn.functional.one_hot(x, num_classes=self.vocab_size - 1).to(torch.float32)
    return x
  ...
  def _maybe_sub_sample(self, x0, attention_mask):
    seqlen = x0.shape[1]
    # print(seqlen, self.config.model.length)
