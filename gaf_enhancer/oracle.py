...
def get_gosai_oracle(mode='train'):
    if mode == 'train':
        model_load = LightningModel.load_from_checkpoint(os.path.join(base_path, 'mdlm/outputs_gosai/lightning_logs/version_0/checkpoints/last.ckpt'), map_location='cuda')
    elif mode == 'eval':
        model_load = LightningModel.load_from_checkpoint(os.path.join(base_path, 'mdlm/outputs_gosai/lightning_logs/version_0/checkpoints/last.ckpt'), map_location='cuda')
    else:
        raise ValueError
...
# def cal_gosai_pred_new(seqs, background_sequences, model=None, mode='eval'):
#     """
#     seqs: list of sequences (detokenized ACGT...)
#     """
#
...
#     preds = model(onehot_tokens.float().transpose(1, 2))
#
#     tokens = dataloader_gosai.batch_dna_tokenize(background_sequences)
#     tokens = torch.tensor(tokens).long().to(device)
#     background_sequences = F.one_hot(tokens, num_classes=4).float()
#
#     explainer = shap.DeepExplainer(model, background_sequences.float().transpose(1, 2))
#     shap_values = explainer.shap_values(onehot_tokens.float().transpose(1, 2), check_additivity=False)
#     shap_values = np.squeeze(shap_values, axis=3)
#     values = shap_values * onehot_tokens.float().transpose(1, 2).cpu().numpy()
#
#
#     return preds.detach().cpu().numpy().squeeze(), values
