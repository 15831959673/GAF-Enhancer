...
def _ga_train(config, logger, file):
  ...
  if file != None:
    ...
    # 取出前10000个适应度最高的元素（如果不足10000个，则取所有元素）
    top_indices = sorted_indices[:10000]  # 若元素不足10000个，会自动取全部

    # 使用排序后的索引从DNA序列中取出对应的元素
...
def modify_config_before_main(i):
  ...
  with initialize(config_path="configs_gosai"):
    ...
    if i == 0:
      ...
      config.checkpointing.resume_ckpt_path = f'/home/zhl/workspace/DRAKES/drakes_dna/data/scratch/wangchy/seqft/mdlm/outputs_gosai/lightning_logs/version_{i - 1}/checkpoints/epoch={(i - 1) * 10 + 9}-step={3930 + (i - 1) * 3538}.ckpt'  # 修改某个键值
    # 传递修改后的配置给main
    return config
